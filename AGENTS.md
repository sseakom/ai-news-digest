...
| Arena 榜基础 URL | `ARENA_BASE` | ~411 |
| Arena 代码榜 URL | `ARENA_LATEST_URL` | ~415 |
| SWE-bench 榜 URL | `SWEBENCH_URL` | ~416 |
| 排行榜展示条数 | `LB_TOP` | ~420 |
| Arena 代码榜抓取 | `fetch_arena_code_leaderboard()` | ~423 |
| SWE-bench agent 榜抓取 | `fetch_swebench_leaderboard()` | ~448 |
| 排行榜格式化 | `build_leaderboard()` | ~471 |
| 拼装最终文本 | `build_text()` | ~512 |
| Server酱推送 | `push_serverchan()` | ~526 |
| ntfy 推送 | `push_ntfy()` | ~556 |
| PushDeer 推送 | `push_pushdeer()` | ~581 |
| 推送入口 (按优先级) | `push()` | ~603 |
...
**Agent 能力** · SWE-bench Verified (真实 Issue 解决率)

| 排名 | Agent 系统 | 解决率 |
|:----:|------------|--------:|
| 🥇 | live-SWE-agent + Claude 4.5 Opus | 79.2% |
| ... | ... | ... |

> 📌 数据来源：Arena AI (LMArena) 代码投票榜 与 SWE-bench Verified 公开榜单。
```
...
**换排行榜源**: 改 `ARENA_LATEST_URL`/`SWEBENCH_URL` 常量和对应 `fetch_*_leaderboard()` 解析逻辑；`LB_TOP` 控制每个榜单展示条数。
