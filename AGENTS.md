...
| 重要程度评分 | `score_item()` | ~127 |
| 日期解析 (含 36氪特殊格式) | `parse_date()` | ~161 |
| RSS/Atom 解析 | `parse_feed()` | ~221 |
| 采集主逻辑 | `collect()` | ~262 |
| DeepSeek 摘要 | `llm_digest()` | ~301 |
| 纯列表格式化 | `plain_list()` | ~367 |
| 拼装最终文本 | `build_text()` | ~394 |
| Server酱推送 | `push_serverchan()` | ~408 |
| ntfy 推送 | `push_ntfy()` | ~438 |
| PushDeer 推送 | `push_pushdeer()` | ~463 |
| 推送入口 (同时) | `push()` | ~485 |
